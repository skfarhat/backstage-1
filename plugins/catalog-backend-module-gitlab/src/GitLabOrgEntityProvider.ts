...
export class GitLabOrgEntityProvider implements EntityProvider {
  ...
  static fromConfig(
    ...
    const result = new GitLabOrgEntityProvider({
      ...
      clientFactory: (target, factoryOptions) => {
        const integration = integrations.gitlab.byUrl(target);
        if (!integration) {
          throw new Error(
            `No integration found for ${target}, please add it to integrations.gitlab in your app-config`,
          );
        }
  ...
  private async readTarget(target: string, logger: Logger, output: Entity[]) {
    try {
      ...
      const client = this.options.clientFactory(target, { logger });
      const users = await readUsers(client, target, {});
      const groupsAdjacency = await readGroups(client, '', '-');
      const groups = [...groupsAdjacency.values()].map(v => v.entity);
